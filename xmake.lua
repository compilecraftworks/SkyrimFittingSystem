...
target("IedConditionIntegrationTests")
    set_default(false)
    set_kind("binary")
    set_encodings("utf-8")
    set_targetdir("build/v" .. build_version .. "/tests")
    add_files("tests/IedConditionIntegrationTests.cpp")
    add_includedirs("src")

target("MenuInteractionInputTests")
    set_default(false)
    set_kind("binary")
    set_encodings("utf-8")
    set_targetdir("build/v" .. build_version .. "/tests")
    add_files("tests/MenuInteractionInputTests.cpp", "src/ui/components/TitleBarHint.cpp",
        "lib/imgui/imgui.cpp", "lib/imgui/imgui_draw.cpp",
        "lib/imgui/imgui_tables.cpp", "lib/imgui/imgui_widgets.cpp")
    add_includedirs("src", "lib/imgui")

target("RenderedOutfitAPITests")
    set_default(false)
    set_kind("binary")
    set_encodings("utf-8")
    set_targetdir("build/v" .. build_version .. "/tests")
    add_files("tests/RenderedOutfitAPITests.cpp")
    add_includedirs("src", "build/.gens/rendered-outfit-tests")
    before_build(function (target)
        os.execv("powershell", {"-NoProfile", "-ExecutionPolicy", "Bypass", "-File",
            ...
            "build/.gens/rendered-outfit-tests"})
    end)
...
target("WornSnapshotRegressionTests")
    set_default(false)
    set_kind("binary")
    set_encodings("utf-8")
    set_targetdir("build/v" .. build_version .. "/tests")
    add_files("tests/WornSnapshotRegressionTests.cpp")
    add_includedirs("src", "build/.gens/worn-snapshot-tests")
    before_build(function (target)
        os.execv("powershell", {"-NoProfile", "-ExecutionPolicy", "Bypass", "-File",
            "tests/Generate-CustomSkinningTestSource.ps1", "-OutputDirectory",
...
for _, regression in ipairs({"RaceMenuHighHeelTests", "CallerChainPerformanceTests"}) do
target(regression)
    set_default(false)
    set_kind("binary")
    set_encodings("utf-8")
    ...
    before_build(function (target)
        os.execv("powershell", {"-NoProfile", "-ExecutionPolicy", "Bypass", "-File",
            "tests/Generate-HighHeelTestSource.ps1", "-OutputDirectory",
            "build/.gens/" .. target:name()})
    end)
end
...
target("AppearanceResourceLifecycleTests")
    set_default(false)
    set_kind("binary")
    set_encodings("utf-8")
    set_targetdir("build/v" .. build_version .. "/tests")
    add_files("tests/AppearanceResourceLifecycleTests.cpp")
    add_includedirs("src", "build/.gens/appearance-resource-tests")
    before_build(function (target)
        os.execv("powershell", {"-NoProfile", "-ExecutionPolicy", "Bypass", "-File",
            "tests/Generate-CustomSkinningTestSource.ps1", "-OutputDirectory",
            "build/.gens/appearance-resource-tests"})
    end)
