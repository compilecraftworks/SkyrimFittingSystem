...
$baseline = Read-Manifest (Join-Path $repo 'Release/Skyrim Fitting System v1.6.1 SE-AE.zip')
...
$dll = Join-Path $repo 'build/v1.6.2/windows/x64/releasedbg/SFSCore.dll'
if ((Get-Item $dll).VersionInfo.FileVersion -ne '1.6.2.0' -or
...
foreach ($name in @('RELEASE-NOTES-v1.6.2.md', 'RELEASE-NOTES-v1.6.2-ko.md',
    'GITHUB-RELEASE-v1.6.2.md', 'nexus-changelog-v1.6.2-en.txt', 'nexus-changelog-v1.6.2-ko.txt',
...
Write-Output 'v1.6.2 verified: DLL/install/locales only; scripts, helper ESL and other runtime assets unchanged.'
