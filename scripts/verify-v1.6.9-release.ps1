...
$runtimePath = Join-Path $repo 'Release/Skyrim Fitting System v1.6.9 SE-AE.zip'
$sourcePath = Join-Path $repo 'Sources/Skyrim Fitting System v1.6.9 Source.zip'
$runtime = Read-Manifest $runtimePath
$source = Read-Manifest $sourcePath
$baseline = Read-Manifest (Join-Path $repo 'Release/Skyrim Fitting System v1.6.8 SE-AE.zip')
$changed = @(@($runtime.Keys) + @($baseline.Keys) | Sort-Object -Unique | Where-Object { $runtime[$_] -ne $baseline[$_] })
$expected = @('SKSE/Plugins/SFSCore.dll',
...
$dll = Join-Path $repo 'build/v1.6.9/windows/x64/releasedbg/SFSCore.dll'
if ((Get-Item $dll).VersionInfo.FileVersion -ne '1.6.9.0' -or
...
foreach ($name in @('RELEASE-NOTES-v1.6.9.md', 'RELEASE-NOTES-v1.6.9-ko.md',
...
Write-Output 'v1.6.9 verified: DLL + three locales only; scripts, helper ESL and other runtime assets unchanged.'
