...
$runtimePath = Join-Path $repo 'Release/Skyrim Fitting System v1.6.6 SE-AE.zip'
$sourcePath = Join-Path $repo 'Sources/Skyrim Fitting System v1.6.6 Source.zip'
$runtime = Read-Manifest $runtimePath
$source = Read-Manifest $sourcePath
$baseline = Read-Manifest (Join-Path $repo 'Release/Skyrim Fitting System v1.6.5 SE-AE.zip')
...
$dll = Join-Path $repo 'build/v1.6.6/windows/x64/releasedbg/SFSCore.dll'
if ((Get-Item $dll).VersionInfo.FileVersion -ne '1.6.6.0' -or
...
foreach ($name in @('RELEASE-NOTES-v1.6.6.md', 'RELEASE-NOTES-v1.6.6-ko.md',
    'GITHUB-RELEASE-v1.6.6.md', 'nexus-changelog-v1.6.6-en.txt', 'nexus-changelog-v1.6.6-ko.txt',
...
Write-Output 'v1.6.6 verified: DLL only; scripts, helper ESL and other runtime assets unchanged.'
