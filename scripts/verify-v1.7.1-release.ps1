...
$runtimePath = Join-Path $repo 'Release/Skyrim Fitting System v1.7.1 SE-AE.zip'
$sourcePath = Join-Path $repo 'Sources/Skyrim Fitting System v1.7.1 Source.zip'
$runtime = Read-Manifest $runtimePath
$source = Read-Manifest $sourcePath
$baseline = Read-Manifest (Join-Path $repo 'Release/Skyrim Fitting System v1.7.0 SE-AE.zip')
...
$dll = Join-Path $repo 'build/v1.7.1/windows/x64/releasedbg/SFSCore.dll'
if ((Get-Item $dll).VersionInfo.FileVersion -ne '1.7.1.0' -or
    (Get-Item $dll).VersionInfo.ProductVersion -ne '1.7.1.0' -or
...
foreach ($name in @('RELEASE-NOTES-v1.7.1.md', 'RELEASE-NOTES-v1.7.1-ko.md',
...
$hashLines = Get-Content -LiteralPath (Join-Path $repo 'Release/SHA256SUMS-v1.7.1.txt')
...
Write-Output 'v1.7.1 verified: DLL only; scripts, helper ESL, locales and other runtime assets unchanged.'
Write-Output 'Corresponding source, tests, scripts, build definitions, release notes, checksums and dist copies match.'
