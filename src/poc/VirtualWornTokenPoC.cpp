...
CaptureWornArmorSnapshot(RE::Actor *a_actor) {
  ...
  for (std::uint32_t slot = 30; slot <= 61; ++slot) {
    ...
    if (sfs::poc::IsDeviousDevicesEquipmentTransactionArmor(armor)) {
      continue;
    }
...
struct NativeDispatchHook {
  static NativeCallResult thunk(
    ...
    if (target == TargetNative::GetWornForm) {
      ...
      static_cast<void>(sfs::poc::UpdateDeviousDevicesHiderSettings(
...
[[nodiscard]] bool InstallNativeDispatchHook(
  ...
  if (!InstallScriptTypeLoadHook(a_vm)) {
    logger::error("Virtual token PoC disabled: generic Papyrus type-load "
                  "observer could not be installed");
    return false;
  }
  if (!InstallNativeRegistrationHook(a_vm)) {
    logger::error("Virtual token PoC disabled: selective Papyrus native "
  ...
  if (patchedCount == 0) {
    logger::error("Virtual token PoC disabled: no selected Papyrus native "
                  "function objects were available");
    return false;
  }
  g_dispatchHookInstalled.store(true, std::memory_order_release);
  logger::warn("Virtual token PoC selective native observer installed for {} "
...
void InitializeVirtualWornTokens() {
  ...
  if (valid && g_dispatchHookInstalled.load()) {
    logger::warn("Trusted virtual ARMO token PoC active: 32 non-inventory "
                 "forms loaded for slots 30-61; generic worn-form, equipped-"
                 "array, filter, and equipment-mutation observation enabled");
  } else if (!valid) {
    logger::error("Virtual token PoC disabled: {} is missing or its records "
                  "failed validation",
                  kTokenPlugin);
  }
}
      ...
      for (const auto &entry :
        ...
        for (const auto &wornEntry :
          ...
          if (savedArmorID == 0 ||
              ...
              !RE::TESForm::LookupByID<RE::TESObjectARMO>(
                  resolvedArmorID) ||
              sfs::poc::IsDeviousDevicesEquipmentTransactionArmor(
                  RE::TESForm::LookupByID<RE::TESObjectARMO>(
                      resolvedArmorID))) {
            continue;
          }
...
void InvalidateVirtualWornTokenAutomationForAppearance(
  ...
  const auto identity = exactAppearance != liveAppearances.end()
                            ...
                            : BuildAppearanceIdentity(
                                  ...
                                  a_slotMask);
  sfs::poc::ReleaseDeviousDevicesHiderSuppressionForAppearanceMask(
      actorID, a_slotMask);
  if (!a_deleted && a_slotMask != 0) {
    auto *actor = RE::TESForm::LookupByID<RE::Actor>(actorID);
    const bool ddHiderControlled =
        actor != nullptr &&
        (sfs::poc::GetDeviousDevicesHiderSuppressedFittingSlotMask(actor) &
         a_slotMask) != 0;
...
void HandleVirtualWornTokenEquipEvent(RE::Actor *a_actor,
  ...
  if (sfs::poc::IsDeviousDevicesEquipmentTransactionArmor(a_armor)) {
  ...
  if (mutation && a_equipped) {
    sfs::poc::ReleaseDeviousDevicesHiderSuppressionForSourceMask(
